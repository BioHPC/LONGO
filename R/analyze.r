...
analyze <- function(DATA.DF,
    ...
    if (FILTERED) {
      isexpr <- rowSums(edgeR::cpm(DATA.DF[2:(ncol(DATA.DF) - 1)]) > 1) >= 4
    ...
    if (NORMALIZED) {
      DATA.DF[2:(ncol(DATA.DF) - 1)] <-
        ...
        as.matrix(DATA.DF[2:(ncol(DATA.DF) - 1)]))
    }

    # sort by length
    DATA.DF$length <- as.numeric(as.character(DATA.DF$length))
    # sort dataframe by length in increasing order. also
    # removes the rows that had no length
    DATA.DF <-
